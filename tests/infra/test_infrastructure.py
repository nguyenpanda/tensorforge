        ...
        with patch("forge_core.benchmark._time_function") as mock_time:
            ...
            compare_and_benchmark(
                ...
                config=BenchmarkConfig(
                    n_runs=1, check_performance=True, silent=False, slowdown_threshold=20.0
                ),
            )
...
class TestHintRegistry:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("basic/01_cpp_integration", "CppIntegration", "run_cpp_addition"),
            ("intermediate/01_cuda_gemm", "CudaGemm", "run_naive_gemm"),
            ("intermediate/01_cuda_gemm", "CudaGemm", "run_tiled_gemm"),
        ],
    )
    def test_method_registered_with_hint_text(self, registry, module_key, class_name, method):
...
class TestASTValidator:
    ...
    def test_custom_feedback_require_calls(self):
        ...
        with pytest.raises(
            BaseException, match="Custom Smart AST Feedback: You must use np.where!"
        ):
            run_test()
